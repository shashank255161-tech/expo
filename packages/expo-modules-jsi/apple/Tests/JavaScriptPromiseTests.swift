...
struct JavaScriptPromiseTests {
  ...
  func `resolve with array`() async throws {
    ...
    #expect(try result[0].getInt() == 1)
    #expect(try result[1].getInt() == 2)
    #expect(try result[2].getInt() == 3)
  }
  ...
  func `promise all`() async throws {
    ...
    #expect(try array.getValue(atIndex: 1).getInt() == 2)
    #expect(try array.getValue(atIndex: 2).getInt() == 3)
  }
